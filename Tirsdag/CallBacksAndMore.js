...
function newTablefunc(){
    ...
    for (let i = 0; i < deezGivenNames.length; i++) {
        myNewMap.push("<tr><td>" + deezGivenNames[i].name + "</td></td>" + deezGivenNames[i].phone + "</td></tr>" );
    }
    myNewMap.unshift("<table>");
    myNewMap.push("</table");
    return myNewMap;
 }
 console.log("4,c)" + newTablefunc().join(""));
 
 function startUp() {
    document.getElementById("tableDiv").innerHTML = newTablefunc();
    document.getElementById("navDiv").innerHTML = newMapfunc();
}
;
